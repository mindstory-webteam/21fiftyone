...
function HoverRollSplitText({ text, className="", delay=50, duration=1.25, ease="power3.out",
  ...
  const units: string[] = splitType==="chars" ? text.split("") : splitType==="words" ? text.split(" ") : text.split("\n");
...
function SplitText({ text, className="", delay=50, duration=1.25, ease="power3.out",
  splitType="chars", from={ opacity:0, y:40 }, to={ opacity:1, y:0 },
  threshold=0.1, rootMargin="-100px", textAlign="left",
  ...
  if (hoverRoll) return <HoverRollSplitText text={text} className={className} delay={delay}
    duration={duration} ease={ease} splitType={splitType} from={from} to={to}
    threshold={threshold} rootMargin={rootMargin} textAlign={textAlign}
...
export default function Hero() {
  ...
  useEffect(() => {
    ...
    mv.src = getVideo(nextIndex); mv.load(); mv.play().catch(()=>{});
  }, [nextIndex]);

  useEffect(() => {
    if (!hasClicked) return;
    const nv = nextVdRef.current; const bg = bgVdRef.current; if (!nv) return;
    ...
    <>
      ...
      <div style={{ position:"relative", height:"100dvh", width:"auto", overflow:"hidden" }}>
        <div id="video-frame" style={{ position:"relative", zIndex:10, height:"100dvh", width:"100vw", overflow:"hidden", background:"var(--black)" }}>
          ...
          {/* Transition video */}
          <video ref={nextVdRef} src={getVideo(currentIndex)} loop muted playsInline
